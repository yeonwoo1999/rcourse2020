...
```{r}
gmp <- read.csv("data/gmp.dat",header = T,sep=" ")
...
```{r}
ggplot(gmp) +
  geom_point(aes(pop,pcgmp)) +
  geom_line(aes(pop,6611*pop^0.125),col="grey",size=1) +
  geom_line(aes(pop,6611*pop^0.15),col="black",size=1) +
  geom_line(aes(pop,6611*pop^0.10),col="pink",size=1) +
  theme_bw()
```
...
```{r}
mse <- function(x,y = gmp$pcgmp,N = gmp$pop){
  result <- mean((y-x[1]*N^x[2])^2)
  return(result)
}
```
```{r}
...
```{r,warning=FALSE}
nlm(mse, c(y_0=6611,a=0.125),y = gmp$pcgmp,N = gmp$pop)
nlm(mse, c(y_0=6611,a=0.15),y = gmp$pcgmp,N = gmp$pop)
nlm(mse, c(y_0=6611,a=0.10),y = gmp$pcgmp,N = gmp$pop)
```
**minimum** represents for the value of the estimated minimum of mse();
**estimate** represents for the point at which the minimum value of mse() is obtained;
Returned values are shown above.
...
plm <- function(x,y = gmp$pcgmp,N = gmp$pop){ 
  # x is a vector which = c(y_0,a)
  result <- nlm(mse,x,y,N)
  final_y_0 <- result$estimate[1]
  final_a <- result$estimate[2]
  final_MSE <- result$minimum
  return(cbind(final_y_0,final_a,final_MSE))
}
plm(c(6611, 0.15))
plm(c(5000, 0.10))
```
y_0 = 6611,a = 0.15 has the lower MSE.
...
jackknifed <- c()
for (i in 1:n) {
  jackknifed[i] <- mean_i(i)
}
jackknifed.mean <- mean(jackknifed)
```
...
```{r}
jackknifed.variance <- (n-1)*mean((jackknifed - jackknifed.mean)^2)
jackknifed.sd <- sqrt(jackknifed.variance)
```
The jack-knife approximation to the standard error of the mean matches very well with the answer in (a).
...
```{r}
plm.jackknife <- function(x,y = gmp$pcgmp,N = gmp$pop){ 
  # x is a vector which = c(y_0,a)
  jackknifed.y_0 <- c()
  jackknifed.a <- c()
  for (i in 1:n) {
    jackknifed.y_0[i] <- plm(x,y[-i],N[-i])[1]
    jackknifed.a[i] <- plm(x,y[-i],N[-i])[2]
  }
  jackknifed.sd.y_0 = sd(jackknifed.y_0)*(n-1)/sqrt(n)
  jackknifed.sd.a = sd(jackknifed.a)*(n-1)/sqrt(n)
  return(cbind(jackknifed.sd.y_0, jackknifed.sd.a))
}
...
plm(c(6611,0.125),y = gmp_2013$pcgmp,N = gmp_2013$pop)
plm.jackknife(c(6611,0.125),y = gmp_2013$pcgmp,N = gmp_2013$pop)
```
